#!/usr/bin/env python3
"""
Step 8.1: Extract Executed Trades

Extracts the actual trades executed for each portfolio strategy, showing the real
currency amounts bought and sold. This process uses the same logic as Step 9 but
outputs individual trade records instead of final portfolio states.

Design:
- Load proposed trades from Step 8
- Load previous portfolio states from Step 9
- Re-execute the trade logic to capture exact amounts
- Output one row per executed trade with full details

This process runs AFTER Step 8 and uses previous day's Step 9 output.
It does not feed into Step 9 - it's a parallel output for trade visibility.

Input:
- data/trades/{date}.csv (from Process 8)
- data/prices/{date}.csv (from Process 1)
- data/portfolios/{previous_date}.csv (from Process 9)
...
def main(date_str=None):
    ...
    try:
        ...
        if all_executed_trades:
            csv_path = write_csv(all_executed_trades, '8.1', date=date_str)
            ...
            csv_path = write_csv([], '8.1', date=date_str)
            logger.add_info('output_file', str(csv_path))
